...
class Ocupabilidad1Controller extends Controller
{
    ...
    public function export_excel(Request $request)
    {
        ...
        ", [$fecha, $fecha, $idmac]);
            ...
            foreach ($resultados as $r) {
                $dias[$dia][$r->IDMODULO] = $r->hora_minima;
            }
        }
        ...
        for ($dia = 1; $dia <= $numeroDias; $dia++) {
            foreach ($modulos as $m) {
                ...
                if (isset($sp[$dia][$m->idmodulo])) {
                    ...
                    $final[$dia][$m->idmodulo] = $dias[$dia][$m->idmodulo];
                } else {
                    $final[$dia][$m->idmodulo] = null;
                }
            }
        ...
        return Excel::download(
            new IndicadorOcupabilidadExport(
                $mesNombre,
                $nombreMac,
                $final,
                $modulos,
            ...
            'INDICADOR_OCUPABILIDAD_' . $nombreMac . '_' . $fecha_año . '_' . $mesNombre . '.xlsx'
        );
    }
}
