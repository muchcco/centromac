...
<script>
    ...
    function _stopAllPolling() {
        if (txtPollingInterval) {
            clearInterval(txtPollingInterval);
            txtPollingInterval = null;
        }
        if (_callaoPolling) {
            clearInterval(_callaoPolling);
            _callaoPolling = null;
        }
    }
    ...
    function _callaoBtn() {
        return $("#modal_show_modal #btnEnviarForm");
    }
        ...
        _callaoPolling = setInterval(function() {
            $.get("{{ route('asistencia.upload.progress') }}", {
                ...
                if (status === 'warning') {
                    ...
                    Swal.fire({
                        icon: 'warning',
                        title: 'Sin registros nuevos para importar',
    ...
    function btnStoreAccess() {
        ...
        if (!file) {
            Swal.fire({
                icon: "warning",
                text: "Selecciona un archivo .mdb o .accdb.",
                confirmButtonText: "Aceptar"
            });
            return;
        }
        ...
        if (ext !== "mdb" && ext !== "accdb") {
            Swal.fire({
                icon: "error",
                text: "Solo se permiten archivos .mdb y .accdb.",
                confirmButtonText: "Aceptar"
            });
            return;
        }
        ...
        if (!fechaInicio || !fechaFin) {
            Swal.fire({
                icon: "warning",
                text: "Completa las fechas de inicio y fin.",
                confirmButtonText: "Aceptar"
            });
            return;
        }
    ...
    function guardarExcepcion() {
        let fecha_inicio = $('#ex_fecha_inicio').val()
        let fecha_fin = $('#ex_fecha_fin').val()
        let mac = $('#ex_mac').val()
        let motivo = $('#ex_motivo').val()
        let valido = $('#ex_valido_hasta').val()
        let revertir_cerrado = $('#ex_revertir_cerrado').is(':checked') ? 1 : 0

        if (!fecha_inicio || !fecha_fin || !mac || !motivo) {
            Swal.fire({
                icon: 'warning',
                title: 'Campos obligatorios',
                text: 'Debe completar Centro MAC, fecha inicio, fecha fin y motivo'
            })
            return
        }
        ...
        let mensaje = 'Se habilitará el cierre fuera de plazo desde <b>' + fecha_inicio + '</b> hasta <b>' + fecha_fin + '</b>'

        if (revertir_cerrado === 1) {
            mensaje += '<br><br><span class="text-danger"><b>También se revertirán los días que ya estén cerrados.</b></span>'
        }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                bloquearBotonExcepcion(true)

                Swal.fire({
                    title: 'Procesando',
                    html: 'Registrando excepción, por favor espere...',
                    allowOutsideClick: false,
                ...
                $.post(
                    "{{ route('asistencia.guardar_excepcion_cierre') }}", {
                        ...
                        motivo: motivo,
                        valido_hasta: valido,
                        revertir_cerrado: revertir_cerrado,
                        _token: "{{ csrf_token() }}"
                    },
                    function(resp) {

                        bloquearBotonExcepcion(false)
                    ...
                    let msg = 'No se pudo registrar la excepción.'
