            ...
            <form id="formAsistenciatest" class="form">
                ...
                <div class="form-group">
                    ...
                    <input type="date" class="form-control" name="fecha" id="fecha1"
                        value="{{ $fecha_asistencia }}" required>
...
<script>
    $(document).ready(function() {
        $('#fecha1').on('change', function() {
            ...
            // 1. Validar que no sea futura
            if (selectedDate > today) {
                $('#fechaError').text("⚠️ La fecha no puede ser posterior a hoy.").show();
                $(this).addClass('is-invalid');
                $('#btnEnviarAsistenciatest').prop('disabled', true);
                return;
            }

            // 2. Validar si el día ya está cerrado/migrado
            $.ajax({
