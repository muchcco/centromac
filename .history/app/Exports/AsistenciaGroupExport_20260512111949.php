...
class AsistenciaGroupExport implements FromView, WithDefaultStyles, ShouldAutoSize,  WithDrawings, WithStyles, WithTitle
{
    ...
    public function view(): View
    {
        ...
        if ($identidad == '17') {
            // 1. Generar array de fechas del rango para completar los días faltantes
            ...
            // 2. Agrupar la data por Colaborador
            $datosAgrupados = collect($data)->groupBy('NUM_DOC')->map(function ($items) {
                return [
                    'encabezado' => $items->first(),
                    'detalle' => $items
                ];
            })->values(); // Convertir a array indexado para evitar problemas de tipos
        }
