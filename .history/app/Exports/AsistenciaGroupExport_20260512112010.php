...
class AsistenciaGroupExport implements FromView, WithDefaultStyles, ShouldAutoSize,  WithDrawings, WithStyles, WithTitle
{
    ...
    {
        $datosAgrupados = $this->datosAgrupados;
        $fechasArray = $this->fechasArray;

        // Solo si llegaron vacíos desde el controlador, los procesamos aquí
        if ($this->identidad == '17' && empty($datosAgrupados)) {

            // 1. Generar array de fechas del rango
            ...
            // 2. Agrupar la data por Colaborador usando $this->query
            $datosAgrupados = collect($this->query)->groupBy('NUM_DOC')->map(function ($items) {
                return [
        ...
        return view('asistencia.exportgroup_excel', [
            ...
            'datosAgrupados' => $datosAgrupados, // Usamos las variables locales procesadas
